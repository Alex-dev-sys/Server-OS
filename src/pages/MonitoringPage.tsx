    ...
    >
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-5">
          ...
          <StatTile icon={Network} label="Network" value={latest.net} unit=" Mbps" color={COLORS.net} />
          <StatTile icon={Gauge} label="Latency" value={latest.ping} unit=" ms" color={COLORS.ping} />
        </div>
        ...
        {/* Network + latency — separate scales, separate charts (never a dual axis) */}
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          <Card>
            <CardHeader
              title="Network Throughput"
              subtitle="fleet average · Mbps"
              icon={
                <span className="grid h-8 w-8 place-items-center rounded-lg" style={{ backgroundColor: `${COLORS.net}1a`, color: COLORS.net }}>
            ...
            <div className="px-2 pb-3 pt-3">
              <MetricChart
                data={data}
                lines={[{ key: 'net', label: 'Network', color: COLORS.net }]}
                rangeMs={range.ms}
                unit=""
                height={200}
              />
          ...
          <Card>
            <CardHeader
              title="Latency"
              subtitle="fleet average · ms"
              icon={
                <span className="grid h-8 w-8 place-items-center rounded-lg" style={{ backgroundColor: `${COLORS.ping}1a`, color: COLORS.ping }}>
            ...
            <div className="px-2 pb-3 pt-3">
              <MetricChart
                data={data}
                lines={[{ key: 'ping', label: 'Latency', color: COLORS.ping }]}
                rangeMs={range.ms}
                unit=""
                height={200}
              />
